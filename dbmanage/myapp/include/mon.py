    ...
    try:
        ...
        if subordinate_status <> 0:
            ...
            if instance.replchannel <> '0':
                subordinate_info=cur.execute("show subordinate status for channel '%s';" %(instance.replchannel))
            else :
                subordinate_info=cur.execute('show subordinate status;')
            result=cur.fetchone()
            # print "result"
            ...
            if instance.check_subordinate:
                if (subordinate_io_run == "Yes") and (subordinate_sql_run == "Yes"):
                    ...
                    alarm_type = 4
                    if record_alarm(instance,alarm_type):
                        sendmail_monitor.delay(instance.id, mysql_monitor.mail_to.split(';'),{'iothread':subordinate_io_run,'sqlthread':subordinate_sql_run}, alarm_type)
        ...
        if result:
            ...
            sql= "update mysql_replication set db_ip=%s,db_port=%s,is_main=%s,is_subordinate=%s,gtid_mode=%s,read_only=%s,main_server=%s,main_port=%s,subordinate_io_run=%s,subordinate_sql_run=%s,delay=%s,current_binlog_file=%s,current_binlog_pos=%s,main_binlog_file=%s,main_binlog_pos=%s,main_binlog_space=%s,subordinate_sql_running_state=%s,create_time where db_ip=%s and db_port=%s"
